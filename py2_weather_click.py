...
def getdata(jsonmsg):
   ...
   tem = BME280_Compensate_Temperature() / 100
   hum = BME280_Compensate_Humidity()  / 1020
   pres = BME280_Compensate_Pressure()  / 100

   jsonmsg = '{"EventNo.":\n{"count": %d "temperature": %d "humidity": %d "pressure": %d }}' % (getdata.count, tem, hum, pres)
   return jsonmsg
getdata.count = 0
